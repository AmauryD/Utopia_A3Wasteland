...
_fn_isUtopiaKeyPressed = {
	private _return = "";
	private _key = _this;

	{
		if(_key in (_x select 1)) then {
			_return = _x select 0;
		};
...
switch (_key call _fn_isUtopiaKeyPressed) do
{
	// U key
	...
	// Tilde (key above Tab)
	case "A3W_customKeys_playerMenu":
	{
...
if(_key in (actionKeys "HeliUp")) then {
	...
	if((typeOf _uav) find "UAV" != -1) then {
		_altitude = getPosASL _uav select 2;
		if(_altitude > 510) then {
			_uav setVectorUp surfaceNormal position _uav;
			_uav setVelocityModelSpace [0, 0,-(_altitude - 510)]; 
			hintSilent "Fly height for drones is limited to 500m";
			_handled = true;
		};
	};
};
