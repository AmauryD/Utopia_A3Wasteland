...
// [utopia] killerViewSystem -> put in a function later ?
_weaponOrVeh = if(vehicle _killer != _killer) then { typeOf vehicle _killer }else{ currentWeapon _killer };
_weaponPic = "";

if(_weaponOrVeh isKindOf "LandVehicle" || _weaponOrVeh isKindOf "Air") then {
	_weaponName = getText (configFile >> "CfgVehicles" >> _weaponOrVeh >> "displayname");
	_weaponPic = getText (configFile >> "CfgVehicles" >> _weaponOrVeh >> "picture");
}else{
	_weaponName = getText (configFile >> "cfgWeapons" >> _weaponOrVeh >> "displayname");
	_weaponPic = getText (configFile >> "cfgWeapons" >> _weaponOrVeh >> "picture");
};

[ 
 "killer_infos", [100,100], [ ["killer_weapon","RscPicture"], ["killer_name","RscText"] ],
 [ 
  ["killer_name",[1,5,15,5]], 
  ["killer_weapon",[1,10,15,12]]
 ]
] call ED_fnc_register;

if (!(_weaponName isEqualTo "") && _killer != _player) then {
	_disp = ["killer_infos","RSC"] call ED_fnc_create;
	_killerNameCtrl = _disp getVariable ["killer_name",controlNull];
	_killerImageCtrl = _disp getVariable ["killer_weapon",controlNull];

	_killerImageCtrl ctrlSetText _weaponPic;
	//_killerAmmo ctrlSetText format ["Used ammo : %1",_ammo];
	_killerNameCtrl ctrlSetText format ["Killed by %1",_killerData select 3];
};

player setVariable ["is_new_profile",true];

[_player, _killerTemp, _presumedKiller, _deathCause,
playerSide,player getVariable "bounty",player //fix this later amaury
] remoteExecCall ["A3W_fnc_serverPlayerDied", 2];
