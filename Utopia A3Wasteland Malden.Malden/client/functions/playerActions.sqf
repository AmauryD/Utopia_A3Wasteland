...
[
	...
	["<img image='client\icons\money.paa'/> Pickup Money", "client\actions\pickupMoney.sqf", [], 1, false, false, "", 
	"{_x getVariable ['owner', ''] != 'mission'} count (player nearEntities ['Land_Money_F', 5]) > 0"],
	...
	[format ["<t color='%1'>[</t>Bug Box<t color='%1'>]</t>", "#00FFFF"],{_this spawn fn_loadDebugBox},[], -20, false],

	["<t color='#3232FF'><img image='client\icons\r3f_contents.paa'/> Items Storage</t>",loadItemBoxMenu,[], 1.2, false, false, "","!isNil {cursorObject getVariable 'VirtualItems'} && player distance cursorObject < 5"],

	["<img image='a3\ui_f\data\Map\Respawn\icon_dead_ca.paa'/>Burry body",{[cursorObject] call ug_fnc_burryDeadBody},[], 1, false, false, "", "cursorObject isKindOf 'man' && !alive cursorObject && player distance cursorObject < 3 && vehicle player == player"],
	["<img image='a3\ui_f\data\Map\Respawn\icon_enemy_ca.paa'/>Body informations",{[cursorObject] call ug_fnc_deadBodyInfos},[], 1.1, false, false, "", "cursorObject isKindOf 'man' && !alive cursorObject && player distance cursorObject < 3 && vehicle player == player"],
