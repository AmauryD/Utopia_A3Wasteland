...
#define GROUPS UG_fnc_dynamicGroups
...
switch (_mode) do
{
	...
	case "DeleteGroup" :
	{
		...
		{
			//["DeleteGroupLocal", [_group]] remoteExecCall ["UG_fnc_dynamicGroups", groupOwner _group];
			[["DeleteGroupLocal", [_group]], "UG_fnc_dynamicGroups", groupOwner _group] call BIS_fnc_mp;
		};
	};
	...
	case "AddKeyEvents" :
	{
		...
		[_display] spawn
		{
			...
			_varName = "UG_dynamicGroups_keyMain";
			...
			if (!isNil { missionNamespace getVariable _varName }) then
			{
				...
				private ["_down", "_up"];
				_down   = _display displayAddEventHandler ["KeyDown", "['OnKeyDown', _this] call UG_fnc_dynamicGroups;"];
				_up     = _display displayAddEventHandler ["KeyUp", "['OnKeyUp', _this] call UG_fnc_dynamicGroups;"];
	...
	case "UpdateKeyDown" :
	{
		...
		if (!isNil { uiNamespace getVariable "UG_dynamicGroups_keyDownTime" } && {count (["GetPlayerInvites", [player]] call GROUPS) > 0}) then
		{
			private ["_timestamp", "_timeHolding"];
			_timestamp      = uiNamespace getVariable "UG_dynamicGroups_keyDownTime";
			...
			if (_timeHolding >= HOLD_DOWN_TIME_FOR_INVITE_ACCEPT) then
			{
				...
				if (count _invites > 0) then
				{
					...
					uiNamespace setVariable ["UG_dynamicGroups_ignoreInterfaceOpening", true];
	...
	case "AddInvite" :
	{
		...
		//["OnInvitationReceived", [_group, _to, _from]] remoteExecCall ["UG_fnc_dynamicGroups", _to];
		[["OnInvitationReceived", [_group, _to, _from]], "UG_fnc_dynamicGroups", _to] call BIS_fnc_mp;

		// If player was kicked from group we unkick since he was invited
		//["UnKickPlayer", [_group, _to]] remoteExecCall ["UG_fnc_dynamicGroups", 2];
		[["UnKickPlayer", [_group, _to]], "UG_fnc_dynamicGroups", false] call BIS_fnc_mp;
	...
	case "OnGroupJoin" :
	{
		...
		{
			[["LocalShowNotification", ["DynamicGroups_PlayerJoined", [name _who], _leader]], "UG_fnc_dynamicGroups", _leader] call BIS_fnc_mp;
		};
	};
	...
	case "OnGroupJoinFailed" :
	{
		...
		if (!isNull _who) then
		{
			[["LocalShowNotification", ["DynamicGroups_PlayerJoinFailed", [], _who]], "UG_fnc_dynamicGroups", _who] call BIS_fnc_mp;
		};
	};
	...
	case "OnGroupLeave" :
	{
		...
		if (!isNull _leader && {!isNull _who} && {_leader != _who}) then
		{
			[["LocalShowNotification", ["DynamicGroups_PlayerLeft", [name _who], _leader]], "UG_fnc_dynamicGroups", _leader] call BIS_fnc_mp;
		};
	};
	...
	case "OnPromotedToLeader" :
	{
		...
		{
			[["LocalShowNotification", ["DynamicGroups_PromotedToLeader", [name _oldLeader], _newLeader]], "UG_fnc_dynamicGroups", _newLeader] call BIS_fnc_mp;
		};
	...
	case "OnGroupDisbanded" :
	{
		...
		{
			[["LocalShowNotification", ["DynamicGroups_GroupDisbanded", [name _oldLeader], _who]], "UG_fnc_dynamicGroups", _who] call BIS_fnc_mp;
		};
	...
	case "OnKicked" :
	{
		...
		{
			[["LocalShowNotification", ["DynamicGroups_Kicked", [name _leader], _who]], "UG_fnc_dynamicGroups", _who] call BIS_fnc_mp;
		};
	...
	case "GetGroupTexture" :
	{
		...
		private _lastInsigniaTexture = _group getVariable ["UG_dynamicGroups_lastinsignia", ""];
		...
		if (_lastInsigniaTexture == "") then
		{
			...
			_insigniaTexture = ["GetInsigniaTexture", [_groupPicture]] call UG_fnc_dynamicGroups;
			_group setVariable ["UG_dynamicGroups_lastinsignia", _insigniaTexture];
		}
		else
		{
