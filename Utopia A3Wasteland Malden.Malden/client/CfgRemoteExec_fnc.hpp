...
class mf_remote_rearm {};

// v1.4
class A3W_fnc_killBroadcast { allowedTargets = 2; };
class A3W_fnc_setVarServer { allowedTargets = 2; };

class ug_fnc_createDrop {
	allowedTargets = 2;
};
class ug_fnc_msgBoxSendMessages {
	allowedTargets = 2;
};
class ug_fnc_msgboxnewmessage {
	allowedTargets = 2;
};
class ug_fnc_msgboxdeletemessage {
	allowedTargets = 2;
};

class ug_fnc_dbSafeVehicleDelete { 
	allowedTargets = 2; 
};
class ug_fnc_dbSafeVehicleSave {
	allowedTargets = 2;
};
class ug_fnc_dbSafeObjectSave {
	allowedTargets = 2;
};
class ug_fnc_dbSafeObjectDelete {
	allowedTargets = 2;
};

class ug_fnc_groupRequestData{
	allowedTargets = 2;
};
class ug_fnc_groupCreate {
	allowedTargets = 2;
};
class ug_fnc_groupInvitePlayer {
	allowedTargets = 2;
};
class ug_fnc_groupJoinPlayer {
	allowedTargets = 2;
};
class ug_fnc_groupKickPlayer {
	allowedTargets = 2;
};
class ug_fnc_groupLeave {
	allowedTargets = 2;
};
class ug_fnc_groupDisband {
	allowedTargets = 2;
};
class ug_fnc_groupNewLeader {
	allowedTargets = 2;
};

class ug_fnc_displayMessage {};
// Bug box
...
// hall of fame
class fn_requestHallOfFameData { allowedTargets = 2; };
