...
ps_create_boxes = {
  ...
  { call {
    ...
    _garage = (nearestObjects [_town_pos, ["Land_i_Garage_V2_F","Land_i_Garage_V1_F", "Land_Shed_06_F", "Land_Warehouse_03_F"], 300]) select 0;
...
if (["A3W_savingMethod", "profile"] call getPublicVar != "sock") then
{
  p_saveStorage = {
    ...
    _containers = (everyContainer _obj) apply {[_x select 0,getMagazineCargo (_x select 1),getWeaponCargo (_x select 1),getItemCargo (_x select 1)]};

    _items = (getItemCargo _obj) call cargoToPairs;
    {
       if((_x select 0) isKindOf ["Uniform_Base",configFile >> "CfgWeapons"]) then {
          _items deleteAt _forEachIndex;
       };
       if((_x select 0) isKindOf ["Vest_Camo_Base",configFile >> "CfgWeapons"]) then {
          _items deleteAt _forEachIndex;
       };
       if((_x select 0) isKindOf ["Vest_NoCamo_Base",configFile >> "CfgWeapons"]) then {
          _items deleteAt _forEachIndex;
       };
    }foreach _items;
  ...
  p_recreateStorageBox = {
...

    {
      ...
      switch (_name) do
      {
        ...
        case "Items": { { _obj addItemCargo _x } forEach _value };
        case "Magazines": { [_obj, _value] call processMagazineCargo };
        case "Containers": { 
          private _i = 0;
          {  
             private _container = _x select 0;
             private _magazines = _x select 1;
             private _weapon = _x select 2;
             private _item =  _x select 3;

             if(_container isKindOf "Bag_Base") then {
              _obj addBackpackCargo [_container,1];
              
             }else{
              _obj addItemCargo [_container,1];
             };
            
             private _containerObj = ((everyContainer _obj) select _i) select 1;

             {_containerObj addMagazineCargo [_x,_magazines select 1 select _forEachIndex]}foreach (_magazines select 0);
             {_containerObj addItemCargo [_x,_item select 1 select _forEachIndex]}foreach (_item select 0);
             {_containerObj addWeaponCargo [_x,_weapon select 1 select _forEachIndex]}foreach (_weapon select 0);
...
ps_inventory_ui_mod = {
  ...
  _ground_tab ctrlSetStructuredText parseText "Storage";
  _ground_tab ctrlCommit 0;

  def(_custom_text);
  _custom_text = _display ctrlCreate ["RscText", -1];
  _custom_text ctrlSetPosition _pos;
  _custom_text ctrlSetText "Private Permanent Storage";
  ...
  def(_custom_description);
  _custom_description = _display ctrlCreate ["RscStructuredText", -1];
  _pos set [0,(_pos select 0) + ((_pos select 2) * 1.2)];
  _pos set [3,(_pos select 3) * 1.5];
  _pos set [2,(_pos select 2) * 2.5];
  _custom_description ctrlSetPosition _pos;
  _custom_description ctrlSetStructuredText parseText "<t size='0.75'>You can access your storage when you respawn , you can add Items to it with the boxes on the map marked with a yellow dot.<br />When you die , it's still in the storage.</t>";
  _custom_description ctrlSetBackgroundColor [0,0,0,0.5];
  _custom_description ctrlSetTextColor [1,1,1,1];
  _custom_description ctrlSetActiveColor [1,1,1,1];
  _custom_description ctrlCommit 0;
