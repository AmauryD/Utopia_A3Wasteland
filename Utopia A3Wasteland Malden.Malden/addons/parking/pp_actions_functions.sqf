...
pp_retrieve_transaction_ok = {
  ...
  [-_cost,_player] call ug_fnc_playerAddMoney;
  true
};
...
pp_park_allowed = {
  ...
  if (isSCALAR(parking_slots) && {parking_slots > 0 && {_count >= parking_slots}}) exitWith {
...
pp_add_actions = {
  ...
  _action_id = _player addAction ["<img image='addons\parking\icons\parking.paa'/> Buy vehicle slot", {
    _this spawn {
      _cost = (pp_retrieve_cost * (parking_slots max 1)) * 50;
      if (parking_slots >= pp_max_player_vehicles_slots) exitWith {hintSilent "You already have the maximum slots"};
      _ok = [format["Do you want to buy a new vehicle slot for %1$ ?",_cost],format["Buy new slot (you have %1)",parking_slots],true,true] call BIS_fnc_guiMessage;
      if (player getVariable "cmoney" < _cost) exitWith {hintSilent "No enough money"};
      if(_ok) then {
          [-_cost] call ug_fnc_playerAddMoney;
