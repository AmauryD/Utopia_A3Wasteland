...
private ["_controllerSuffix", "_missionTimeout", "_availableLocations","_missionCompleter", "_missionLocation", "_leader", "_marker", "_failed", "_complete", "_startTime", "_oldAiCount", "_leaderTemp", "_newAiCount", "_adjustTime", "_lastPos", "_floorHeight"];

// Variables that can be defined in the mission script :
private ["_missionType", "_locationsArray", "_aiGroup", "_missionPos", "_missionPicture", "_missionHintText", "_successHintMessage", "_failedHintMessage","_missionMarkerColor"];
...
if (isNil "_missionPicture") then { _missionPicture = "\A3\ui_f\data\IGUI\Cfg\MPTable\infantry_ca.paa" };
if (isNil "_missionMarkerColor") then { _missionMarkerColor = "ColorRed" };


_leader = leader _aiGroup;
_marker = [_missionType, _missionPos , _missionMarkerColor,_missionPicture] call createMissionMarker;
_aiGroup setVariable ["A3W_missionMarkerName", _marker, true];

if(!isNil "_vehicle") then {
	_aiGroup setVariable ["A3W_missionVehicle",_vehicle,true];
};
...
else
{
	...
	_missionCompleter =  leader _aiGroup getVariable ["killer",objNull];
