...
private ["_MoneyShipment", "_moneyAmount", "_convoys", "_vehChoices","_missionType", "_moneyText", "_vehClasses", "_createVehicle", "_vehicles", "_veh2", "_leader", "_speedMode", "_waypoint", "_vehicleName", "_numWaypoints", "_cash"];
...
_setupVars =
{
	...
	[
		...
		[
			"Super Heavy Money Shipment",
			100000,
			[
				[ // NATO convoy
					["B_T_MBT_01_TUSK_F"], // Veh 1
					["B_T_MBT_01_TUSK_F"], // Veh 2
					["B_T_MBT_01_TUSK_F"], // Veh 3
					["B_T_APC_Tracked_01_AA_F"] // Veh 4
				]
			]
		]
	]
...
_setupObjects =
{
	...
	_speedMode = if (_missionType == "Super Heavy Money Shipment") then { "NORMAL" } else { "LIMITED" };
	...
	if(_missionType == "Super Heavy Money Shipment") then {
		{_x setSkill 1} foreach units _aiGroup;
		{_x allowCrewInImmobile true}foreach _vehicles;
	};
};
...
_successExec =
{
	...
	_successHintMessage = "The convoy has been stopped, the money are now yours to take.";
};
